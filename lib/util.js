  ...
  try {
    if (rowStatus === 'added') {
      ...
      if (rowStatus === 'modified') {
        model.replaceOrCreate(r, options, function (err, instance) {
          cb(err, instance);
        });
      } else {
      ...
      return cb(new Error('No primar key defined'));
    }
  } catch (err) {
    return cb(err);
  }
...
module.exports.getChildRelations = function getChildRelations(model) {
  ...
  for (var r in model.relations) {
    if (model.relations[r].type === 'hasMany' || model.relations[r].type === 'hasOne' || self.relations[r].type === 'belongsTo') {
